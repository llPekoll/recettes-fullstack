...
const addIngredient = async (page: Page, quantity: string, unit: string, name: string): Promise<void> => {
    ...
    await sleep(1000);
    await expect(page.locator(`[data-test="ing-${name}"]`).first()).toBeVisible()
};
...
const addStep = async (page: Page, title: string): Promise<void> => {
    ...
    await sleep(1000);
    await expect(page.locator(`[data-test="step-${title}"]`).first()).toBeVisible()
}


export const createRecipe = async (page: Page, basic: boolean, publish: boolean): Promise<void> => {
    await page.locator('[data-test="create-recipe"]').click()
    await page.locator('[data-test="title"]').fill("My recipe")
    if (!basic) {
